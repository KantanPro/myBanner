...
- フロントページ・投稿インデックスへの自動表示（設定でオン/オフ）
...
### 1) フロントページ・投稿インデックスへ自動表示

設定画面で以下を有効にします。

- **フロントページで自動表示** … サイトのフロントページに表示
- **投稿インデックスで自動表示** … ブログ投稿一覧（インデックス）に表示
- **自動表示位置** … `wp_footer` または `wp_body_open`
...
1. **外観 > ウィジェット** を開く
2. **myBanner** をサイドバー等のウィジェットエリアに追加
